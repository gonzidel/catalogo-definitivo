-- 350_rpc_admin_add_order_items_allow_sent.sql
--
-- Fix 2026-09-18: admin/sent-orders.html → "Agregar productos" fallaba con
-- ORDER_STATE_BLOCKED porque 347 rechazaba status `sent`.
--
-- Operación legítima: sumar ítems a un pedido ya marcado enviado (olvido en
-- el bulto) sin reabrirlo a active/closed. El UPDATE de 347 ya preservaba
-- cualquier estado fuera de active/closing_soon; solo faltaba la allowlist.
--
-- Sigue bloqueado: cancelled, expired, stock_pending, devolución.
--
-- Base: 347_rpc_admin_add_order_items_atomic.sql (mismo cuerpo + allowlist).
-- Rollback: 350_ROLLBACK_rpc_admin_add_order_items_allow_sent.sql (vuelve a 347).

-- 347_rpc_admin_add_order_items_atomic.sql (cuerpo reutilizado)
--
...
COMMENT ON FUNCTION public.rpc_admin_add_order_items_atomic(uuid, jsonb, uuid) IS
  'canonical:350 | Admin: agrega ítems/stock/total en una TX. Allowlist: active, closing_soon, closed, sent. Bloquea cancelled/expired/stock_pending/devolución.';
