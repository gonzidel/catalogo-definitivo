...
-- 10) Función RPC para mover stock por talle específico entre almacenes
...
begin
  ...
  -- Convertir a string y remover espacios, luego truncar decimales si es número
  ...
  -- Verificar stock disponible en origen para este talle específico
  select coalesce(stock_qty, 0) into v_available_stock
  ...
  if v_available_stock < p_quantity then
    raise exception 'Stock insuficiente en almacén origen para talle %. Disponible: %, Solicitado: %', 
  ...
  -- Registrar movimiento en historial (sin size, ya que stock_movements no tiene esa columna)
  insert into public.stock_movements (
    variant_id, 
    from_warehouse_id, 
    to_warehouse_id, 
  ...
  values (
    ...
    p_quantity,
    v_user_id,
    coalesce(p_notes || ' (Talle: ' || v_normalized_size || ')', 'Talle: ' || v_normalized_size)
  )
  returning id into v_movement_id;
...
begin
  ...
  -- Buscar pedido abierto del cliente (active o closing_soon)
  select id
    into v_order_id
    from public.orders
   where customer_id = auth.uid()
