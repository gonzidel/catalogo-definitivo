...
export default function OrderEditModal({ order, onClose }: OrderEditModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // No alcanza con refetch+patch: si la clienta ya había pedido cerrar
      // (customer_requested_close) y este guardado dejó el pedido completo
      // (ej. se agregó el último ítem faltante ya apartado), hay que cerrarlo
      // acá mismo -- sin este chequeo quedaba trabado en Apartados para
      // siempre esperando que alguien lo cierre a mano.
      const { order: refreshed, autoClosed } = await refreshAndMaybeAutoClose(supabase, liveOrder.id);
      if (refreshed) {
      ...
      setDraft([]);
      showToast(autoClosed ? "Pedido actualizado y cerrado (la clienta ya lo había enviado)" : "Pedido actualizado", "success");
                ...
                {items.length === 0 && draft.length === 0 && noteExtraRows.length === 0 ? (
                  ...
                  <ul className="order-edit-modal__list order-edit-modal__list--compact">
                    {savedProducts.map((item) => (
                      <li
                        key={item.id}
                        className="order-edit-modal__row order-edit-modal__row--compact order-edit-modal__row--split"
                      >
                        ...
                        <div className="order-edit-modal__compact-actions">
                          ...
                          <span className="order-edit-modal__compact-cell order-edit-modal__compact-cell--price">
                            {formatPriceAr(getOrderItemLineTotal(item))}
                          </span>
                          <button
                            type="button"
                    ...
                    {draft.map((item, idx) => {
                      ...
                      return (
                        <li
                          ...
                          className={`order-edit-modal__row order-edit-modal__row--compact order-edit-modal__row--split order-edit-modal__row--draft${special ? " order-edit-modal__row--special" : ""}`}
                        >
                          <div className="order-edit-modal__compact-label">
                            {special ? (
                              <>
                              ...
                              <>
                                ...
                                <span className="order-edit-modal__compact-meta">
                                  {item.size || "-"} ×{item.quantity} · pendiente
                                </span>
                              </>
                            )}
                          </div>
