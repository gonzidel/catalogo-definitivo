-- EJECUTAR_FUNCION_IMPORTACION.sql
-- Ejecuta este script en Supabase SQL Editor para crear la función de importación masiva
-- IMPORTANTE: Ejecuta este script ANTES de usar los scripts de importación
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Si la función generate_customer_number no existe, usar un número secuencial simple
      BEGIN
        SELECT public.generate_customer_number() INTO v_customer_number;
      EXCEPTION WHEN OTHERS THEN
        -- Si no existe la función, generar número manualmente
        ...
        v_customer_number := LPAD(v_customer_number::text, 4, '0');
      END;
...
GRANT EXECUTE ON FUNCTION public.rpc_bulk_create_customers(jsonb) TO authenticated, service_role;
