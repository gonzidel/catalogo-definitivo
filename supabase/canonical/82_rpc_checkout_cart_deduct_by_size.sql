...
-- Evita que el stock por talle no se descuente cuando el cliente hace pedido.
...
BEGIN
  ...
  LOOP
    ...
    -- Si el ítem tiene size, descontar de variant_size_warehouse_stock (general primero, luego venta público)
    ...
    IF v_size_normalized != '' AND v_general_id IS NOT NULL AND v_venta_id IS NOT NULL THEN
      ...
      END IF;
    ELSE
      -- Sin size: descontar de variant_warehouse_stock (general primero, luego venta público)
