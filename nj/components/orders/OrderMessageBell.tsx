...
import { loadBellSentIds, persistBellSentId } from "@/lib/orders/bell-sent";
...
function NotifCard({
  ...
  if (sent) {
    cardClass += " order-msg-bell__card--sent";
  } else if (isExpiry) {
  ...
  return (
    <article className={cardClass}>
      ...
      {isClosed ? <span className="order-msg-bell__card-badge">Cerrado</span> : null}
      ...
      <div className="order-msg-bell__card-actions">
        ...
        <button
          ...
          title={sent ? "Ya enviado — se puede reenviar" : "Enviar por WhatsApp"}
          onClick={onSend}
        >
          Enviar mensaje
        </button>
...
export default function OrderMessageBell({ boardScope }: OrderMessageBellProps) {
  ...
  const [sentIds, setSentIds] = useState<Set<string>>(() => loadBellSentIds());
  ...
  const notificationBelongsToBoard = useMemo(() => {
    return (notif: OrderMsgNotification): boolean => {
      ...
      if (boardScope === "shipping" && inboxView !== "general") {
        return orderMatchesKanbanInboxView(order, inboxView, { boardScope });
      }
      return true;
    };
  }, [boardScope, ordersById, warehouseIds, inboxView]);
  ...
  const markCardSent = (id: string) => {
    setSentIds(persistBellSentId(id));
  };
  ...
  return (
    <div className="order-msg-bell">
      <button
        ...
        aria-label={
          count > 0 ? `Notificaciones de mensajes: ${count}` : "Notificaciones de mensajes"
        }
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
      >
