// scripts/main.js - Versión optimizada del catálogo principal
  ...
  renderColors(product) {
    return product.DetalleColor.map((v) => {
      ...
      return `<button class='color-btn' 
                      data-src="${v.images[0]}" 
                      style="background-color: ${hexColor}; color: ${textColor}; border: 1px solid ${hexColor};">${v.color}</button>`;
    }).join("");
  }
...
async function downloadImage(btn) {
  ...
  try {
    ...
    const a = document.createElement("a");
    a.href = src;
    a.download = filename || "catalogo-fyl.jpg";
    document.body.appendChild(a);
    a.click();
    a.remove();
  }
