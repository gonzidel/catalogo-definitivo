-- 08_cart_items_flexible_fixed.sql - Versión corregida que elimina funciones existentes
...
-- Eliminar funciones existentes que pueden causar conflictos
DROP FUNCTION IF EXISTS get_cart_items_simple(uuid);
DROP FUNCTION IF EXISTS get_user_cart(uuid);
DROP FUNCTION IF EXISTS clear_cart_items(uuid);
DROP FUNCTION IF EXISTS add_cart_item(uuid, text, text, text, int, numeric);
...
-- Función simple para obtener items del carrito (con imagen)
...
CREATE OR REPLACE FUNCTION add_cart_item(
    ...
    quantity_param int,
    price_param numeric,
    imagen_param text DEFAULT NULL
)
RETURNS uuid AS $$
DECLARE
    item_id uuid;
...
-- Crear vista simple para carrito del usuario (con imagen)
