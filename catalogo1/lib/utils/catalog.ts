...
/** Variante visible solo si tiene al menos una URL de imagen propia. */
export function colorDetailHasImage(dc: ColorDetail): boolean {
  return (dc.images ?? []).some((img) => Boolean(String(img ?? "").trim()));
}
