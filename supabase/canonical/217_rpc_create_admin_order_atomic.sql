-- 217_rpc_create_admin_order_atomic.sql
--
-- Primera versión atómica: alta pedido admin + order_items + inject (179) + deducción (166)
-- + idempotencia (tabla 215) + helpers (216).
--
-- NO reemplaza order-creator.js / createNewOrder.
-- NO altera el cuerpo de rpc_apply_order_stock_deduction ni rpc_admin_manual_inject_and_deduct.
--
-- Rollback: 217_ROLLBACK_rpc_create_admin_order_atomic.sql
...
comment on function public.rpc_create_admin_order_atomic(jsonb, uuid) is
  'canonical:217 — v1 staging: alta pedido admin atómica + idempotencia (ver doc/plan-implementacion-rpc-create-admin-order-atomic-staging-2026-05-15.md).';

revoke all on function public.rpc_create_admin_order_atomic(jsonb, uuid) from public, anon, authenticated;
grant execute on function public.rpc_create_admin_order_atomic(jsonb, uuid) to authenticated, service_role;
