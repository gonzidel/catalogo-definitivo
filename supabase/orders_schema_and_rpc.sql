-- Supabase Orders: carts, cart_items, RLS y RPC (idempotente)
-- Copiar/pegar completo en SQL Editor y ejecutar una sola vez.

-- 0) Utilidades y columnas necesarias ---------------------------------------
...
-- Asegurar columna de reservas en variantes
...
create table if not exists public.carts (
  ...
  status text not null default 'open', -- open, submitted, confirmed, ready_to_ship, closed, canceled
...
create table if not exists public.cart_items (
  ...
  status text not null default 'reserved', -- reserved, confirmed, rejected
...
-- Cliente autenticado: ver/crear/editar solo sus propios carritos/ítems
do $$
begin
  if not exists (
    select 1 from pg_policies where schemaname='public' and tablename='carts' and policyname='carts_self_access'
  ) then
    create policy carts_self_access
...
begin
  if not exists (
    select 1 from pg_policies where schemaname='public' and tablename='cart_items' and policyname='cart_items_self_access'
  ) then
    create policy cart_items_self_access
...
-- Admin (public.admins): acceso total
do $$
begin
  if not exists (
    select 1 from pg_policies where schemaname='public' and tablename='carts' and policyname='carts_admin_access'
  ) then
    create policy carts_admin_access
      on public.carts for all to authenticated
...
begin
  if not exists (
    select 1 from pg_policies where schemaname='public' and tablename='cart_items' and policyname='cart_items_admin_access'
  ) then
    create policy cart_items_admin_access
      on public.cart_items for all to authenticated
      using (exists (select 1 from public.admins a where a.user_id = auth.uid()))
      with check (exists (select 1 from public.admins a where a.user_id = auth.uid()));
  end if;
...
-- 4) RPC: obtener/crear carrito abierto -------------------------------------
...
begin
  ...
  if cid is null then
    insert into public.carts(id, customer_id, status)
      values (gen_random_uuid(), auth.uid(), 'open')
...
begin
  ...
  -- Bloquear fila y chequear available
...
-- 7) RPC: admin confirma/rechaza ítem ---------------------------------------
...
begin
  ...
  if new_status = 'confirmed' then
    ...
    select variant_id, qty into v_variant, v_qty
...
begin
  ...
  for r in
    select variant_id, qty
      from public.cart_items
     where cart_id = cid and status = 'confirmed'
  loop
    update public.product_variants
...
-- 9) Recargar el esquema del API REST ---------------------------------------
