-- 223_add_fecha_publicacion_to_catalog_views.sql
-- Expone last_published_at (variante > producto) como "FechaPublicacion" para orden del index.
-- Alineado con admin/publications.js (publishSelected actualiza product_variants + products).
--
-- DEPLOY (staging-first):
--   1) Este archivo
--   2) select public.fyl_rebuild_catalog_public_snapshot_parity(true);
--      -- o rpc_refresh_catalog_public_snapshot() con JWT admin
--
-- Rollback: 223_ROLLBACK_add_fecha_publicacion_to_catalog_views.sql
...
alter table public.catalog_public_snapshot
  add column if not exists "FechaPublicacion" timestamptz;

-- Tras apply: paridad + refresh (admin Acciones rápidas o):
