...
- **2026-08-15 (2):** named transformations en Cloudinary `t_fyl_mini|thumb|sm|card|pdp|hero|blur` (`allowed_for_strict`). Loader Next usa `t_fyl_*/f_auto/q_auto` y hace snap al ancho canónico. Purge de definiciones 2048/3840 (varias); `f_auto,q_auto,c_scale,w_3840` no se pudo borrar por volumen (“too many derived”) — hay que purgarlo en consola tras el deploy. No se tocaron w_1920 todavía (siguen en producción hasta el deploy).

## Deuda restante

1. Named transformations (`t_card`, `t_pdp`, `t_thumb`) + valorar Strict Transformations.
2. `invalidate` solo si el binario cambió.
3. Purge de derived viejos (w_1920/2048/3840/JXL) en consola para bajar storage.
