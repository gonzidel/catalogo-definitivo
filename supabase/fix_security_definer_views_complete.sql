-- fix_security_definer_views_complete.sql
-- Script COMPLETO para eliminar SECURITY DEFINER de vistas
-- Incluye cambio de owner si es necesario
-- Ejecutar este script en el SQL Editor de Supabase

-- ============================================================================
-- CORRECCIÓN COMPLETA: Eliminar SECURITY DEFINER de vistas
-- ============================================================================

-- NOTA: No cambiamos el owner porque puede causar errores de permisos.
-- Las vistas en PostgreSQL por defecto son SECURITY INVOKER (no DEFINER),
-- así que solo necesitamos recrearlas correctamente.

-- ============================================================================
-- 1. ELIMINAR Y RECREAR catalog_public_view
-- ============================================================================
...
-- Verificar owner de las vistas (información)
SELECT 
    n.nspname AS schema_name,
    c.relname AS view_name,
    r.rolname AS owner_name,
    CASE 
        WHEN r.rolsuper THEN '⚠️  Owner es superusuario (normal en Supabase)'
...
-- Verificar que las vistas no tienen SECURITY DEFINER en su definición
SELECT 
    schemaname,
    viewname,
    CASE 
        WHEN pg_get_viewdef((schemaname||'.'||viewname)::regclass) LIKE '%SECURITY DEFINER%' 
        THEN '⚠️  Tiene SECURITY DEFINER en definición'
...
BEGIN
  ...
  RAISE NOTICE '   1. Vistas recreadas sin SECURITY DEFINER';
  RAISE NOTICE '   2. Vistas configuradas como SECURITY INVOKER (por defecto)';
  RAISE NOTICE '   3. Permisos otorgados correctamente';
  RAISE NOTICE '';
  RAISE NOTICE '⏳ Próximos pasos:';
  RAISE NOTICE '   - Espera 2-5 minutos para que Supabase Advisor se actualice';
  RAISE NOTICE '   - Si el error persiste, puede ser un falso positivo';
  RAISE NOTICE '   - Las vistas ahora respetan RLS del usuario que consulta';
  RAISE NOTICE '═══════════════════════════════════════════════════════════';
END $$;
