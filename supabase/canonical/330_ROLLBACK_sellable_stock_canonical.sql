-- 330_ROLLBACK_sellable_stock_canonical.sql
-- Restaura catalog_public_available_view a la fórmula 223 (resta OISS + cart reserved)
-- y elimina las funciones canónicas de sellable.
-- NO refresca snapshot. NO toca rpc_checkout_cart.
...
comment on view public.catalog_public_available_view is
  'Catálogo público (fórmula 223): available = físico web − OISS − cart reserved.';
...
drop function if exists public.fn_sellable_stock_batch(uuid[]);
drop function if exists public.fn_sellable_qty(uuid, text);
drop function if exists public.fn_norm_size(text);
